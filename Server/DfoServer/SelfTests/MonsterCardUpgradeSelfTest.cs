...
");
            ...
            Check("white same-rarity chance is 70%", config.CalculateChance(0, 0) == 70000, ref failures);
            Check("blue same-rarity chance is 60%", config.CalculateChance(1, 1) == 60000, ref failures);
            Check("purple same-rarity chance is 50%", config.CalculateChance(2, 2) == 50000, ref failures);
            Check("pink same-rarity chance is 40%", config.CalculateChance(3, 3) == 40000, ref failures);
            Check("higher-rarity material is guaranteed", config.CalculateChance(2, 3) == 100000, ref failures);
            Check("one tier lower halves chance", config.CalculateChance(3, 2) == 20000, ref failures);
            Check("three tiers lower yields 5%", config.CalculateChance(3, 0) == 5000, ref failures);
            ...
            Check("same-slot failure consumes only one card", CheckSameSlotFailure(config), ref failures);
        ...
        private static bool CheckSameSlotFailure(MonsterCardUpgradeConfig config)
        {
            var inventory = CreateInventory(3);
